...
def test_openmm():
    ...
    try:
        ...
        if major < 7 or (major == 7 and minor < 7):
            print(f"  ⚠️  OpenMM {version} is outdated")
        ...
        if 'CUDA' in platforms:
            ...
            prop_names = [cuda_platform.getPropertyName(i) for i in range(cuda_platform.getNumProperties())]
...
def test_pymbar():
    ...
    try:
        ...
        try:
            ...
            # Test functions (pymbar 4.x uses snake_case and needs numpy arrays)
            test_data = np.array([1.0, 1.1, 1.0, 0.9, 1.0] * 100)  # Fake correlated data
            t0, g, Neff = timeseries.detect_equilibration(test_data)
            print(f"    Test: detect_equilibration() works (t0={t0}, g={g:.1f}, Neff={Neff:.0f})")
